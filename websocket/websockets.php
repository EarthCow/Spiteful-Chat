...
abstract class WebSocketServer
{
      ...
      foreach ($read as $socket) {
        if ($socket === $this->master) {
          ...
          if ($client === false) {
            ...
            $this->connect($client);
            //$this->stdout('Client observed. ' . strval(spl_object_id($client)) );
          }
        } else {
  ...
  protected function doHandshake($user, $buffer)
  {
    ...
    if (
      ($this->headerOriginRequired && !isset($headers["origin"])) ||
    ...
    if (isset($handshakeResponse) && !empty($handshakeResponse)) {
      $this->stdout("\033[36m[i] \033[39m" . $handshakeResponse);
      socket_write(
        $user->socket,
        $handshakeResponse,
  ...
  protected function split_packet($length, $packet, $user)
  {
    ...
    while ($frame_pos < $length) {
      ...
      if (
        ($message = $this->deframe($frame, $user, $headers)) !== false
      ) {
        if ($user->hasSentClose) {
          ...
          if (
            ...
            $this->stderr("\033[31m[!] \033[39mNot UTF-8\n");
          }
        }
      }
  ...
  protected function deframe($message, &$user)
  {
    ...
    $payload =
      $user->partialMessage . $this->extractPayload($message, $headers);
  ...
  protected function extractHeaders($message)
  {
    ...
    $header["length"] =
      ord($message[1]) >= 128 ? ord($message[1]) - 128 : ord($message[1]);
